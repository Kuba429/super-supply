...
export default function ItemCard({ item }: { item: itemType }) {
	return (
		<Link
			href={"item/" + item.id}
			className="relative flex h-80 w-52 flex-shrink-0 cursor-pointer snap-center flex-col rounded bg-neutral-100 shadow-md after:absolute after:bottom-0 after:left-0 after:-z-10 after:h-0 after:w-[calc(100%_+_4px)] after:translate-x-[-2px] after:translate-y-[2px] after:rounded after:bg-red-500 after:transition-all hover:after:h-[calc(100%_+_4px)]"
		>
			<Image
				src={item.image}
				width={210}
